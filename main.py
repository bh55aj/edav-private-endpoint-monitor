#!/usr/bin/env python3
"""
EDAV Private Endpoint Monitor v4.1.0
=======================================
...
What this does
--------------
* Validates Azure login before any work begins (exits with clear instructions)
...
* Generates colour-coded Excel/CSV/HTML/JSON/Markdown reports with auto-sized columns
* Post-delete verification: confirms endpoint is gone after every deletion
* Executive dashboard printed at end of every run
* Live progress spinner during Azure CLI calls
* Startup banner with run metadata
* Complete run summary footer listing all output files with sizes
* Optionally emails all reports
...
_BANNER = r"""
  ...
  ███████╗██████╔╝██║  ██║ ╚████╔╝     ██║     ███████╗
  ╚══════╝╚═════╝ ╚═╝  ╚═╝  ╚═══╝      ╚═╝     ╚══════╝
  Private Endpoint Monitor  v{version}  |  Enterprise Azure Governance
"""

def print_banner(version: str, mode: str, run_dt: str,
                 change_ticket: str = "", approved_by: str = ""):
    """Print a formatted startup banner to console."""
    banner = _BANNER.format(version=version)
    width = 72
    sep  = "=" * width
    sep2 = "-" * width
    print("")
    ...
    print(sep2)
    print(f"  Tool         : EDAV Private Endpoint Monitor v{version}")
    print(f"  Safety       : 14-layer deletion gate | backup-before-delete | post-delete verify")
...
def phase_start(num: int, label: str):
    """Print a phase header and record start time."""
...
class Spinner:
    """Lightweight console spinner for long Azure CLI calls.
    Usage:  with Spinner("Scanning..."):  do_work()
    Spinner stops automatically on exit (success or exception).
    """
    ...
    def __enter__(self):
        # Only spin if stdout is a real terminal
...
def validate_azure_login() -> dict:
    ...
    try:
        ...
        if r.returncode == 0 and r.stdout.strip():
            ...
            return acct
        # Non-zero returncode -- not logged in
        err = (r.stderr or r.stdout or "").strip()
        log.debug("az account show non-zero: %s", err[:200])
    except Exception as exc:
...
def verify_endpoint_deleted(name: str, rg: str, sub: str) -> tuple:
    """
    Post-delete verification. After every deletion, confirm Azure returns
    ResourceNotFound for the endpoint.
...
def run_post_delete_verification(del_log: list, dry_run: bool = False) -> list:
    ...
    DRY-RUN entries  -> 'Verification Skipped (Dry Run)'
    Deleted entries  -> calls verify_endpoint_deleted(), records result
    ...
    for rec in other_entries:
        verify_log.append({
            ...
            "Verification Timestamp": datetime.now().isoformat(),
            "Verification Notes":  ("Delete result was '"
                                    + str(rec.get("Delete Result", ""))
    ...
    v_skip = sum(1 for v in verify_log
                 if "Skipped" in v["Verification Status"] or "Not Applicable" in v["Verification Status"])
...
def load_endpoints(path: str) -> list:
    ...
    Handles parser errors, empty files, and permission issues gracefully."""
    path = path.strip()
    if not os.path.isfile(path):
    ...
    try:
        ...
        log.error("Permission denied reading input file: %s", path)
        log.error("Close the file if it is open in Excel, then retry.")
        sys.exit(1)
    except pd.errors.EmptyDataError:
        log.error("Input file is empty: %s", path)
        sys.exit(1)
    except pd.errors.ParserError as exc:
        log.error("CSV/Excel parse error in %s: %s", path, exc)
        sys.exit(1)
    except Exception as exc:
    ...
    if "Endpoint Name" not in df.columns:
        log.error("No 'Endpoint Name' column found. Columns detected: %s", list(df.columns))
        sys.exit(1)
...
def scan(ep: dict, subscriptions: list, tf_state: str,
    ...
    Per-subscription errors are isolated -- one failing sub never crashes the scan."""
    if denylist is None:
        denylist = set()
    ...
    for sub in subscriptions:
        try:
            ...
            log.warning("  [SUB-ERROR] Subscription '%s' error for '%s': %s", sub, name, exc)
            continue
...
def _is_deletion_blocked(rec: dict, exclusions: set, denylist: set = None) -> tuple:
    ...
    6. Recommended Action contains a blocking keyword
    """
    if denylist is None:
        denylist = set()
...
def _autosize_sheet(ws, headers: list, extra_padding: int = 4):
    """Auto-size all columns based on header + data content."""
        ...
        for row in ws.iter_rows(min_row=2, min_col=ci, max_col=ci):
            for cell in row:
                try:
                    ...
                    pass
        # Cap column width at 60 chars, minimum 12
        adjusted = min(max(max_len + extra_padding, 12), 60)
        ws.column_dimensions[col_letter].width = adjusted

def _set_row_height(ws, start_row: int = 1, height: float = 18.0):
    """Set a consistent row height for all data rows."""
    for row in ws.iter_rows(min_row=start_row):
        ws.row_dimensions[row[0].row].height = height

# ===========================================================================
# Validation / Discovery report (XLSX) -- with auto-sized columns
# ===========================================================================
...
# ===========================================================================
# Delete report (XLSX) -- with auto-sized columns
# ===========================================================================
...
# ===========================================================================
# Verification report (XLSX) -- with auto-sized columns
# ===========================================================================
...
# ===========================================================================
# HTML report builder -- enhanced in v4.1.0
# ===========================================================================
...
def build_html_report(results: list, del_log: list, verify_log: list,
    ...
    def scan_rows_html():
        ...
        for r in results:
            ...
            rows += f'<td>{r.get("Endpoint Name","")}</td>'
    ...
    def del_rows_html():
        if not del_log:
            return "<tr><td colspan=6 style='color:#888'>No deletion operations performed</td></tr>"
        rows = ""
        for r in del_log:
    ...
    def ver_rows_html():
        if not verify_log:
            return "<tr><td colspan=5 style='color:#888'>No verification data (verify-only mode)</td></tr>"
        rows = ""
        for v in verify_log:
            status = v.get("Verification Status", "")
            colour = ("#C6EFCE" if status.startswith("Verified")
                      else "#FFC7CE" if "FAILED" in status else "#D9D9D9")
            rows += f'<tr style="background:{colour}">'
            rows += f'<td>{v.get("Endpoint Name","")}</td>'
            rows += f'<td>{v.get("Resource Group","")}</td>'
            rows += f'<td><b>{status}</b></td>'
            rows += f'<td>{v.get("Azure Response","")}</td>'
    ...
    def summary_table_html():
        rows = ""
        for action, count in sorted(counts.items(), key=lambda x: x[1], reverse=True):
            colour = row_colour(action)
            pct    = f"{count/total*100:.1f}%"
            rows += (f'<tr style="background:{colour}">'
                     f'<td>{action}</td>'
                     f'<td style="text-align:center;font-weight:bold">{count}</td>'
                     f'<td style="text-align:center">{pct}</td></tr>')
        return rows

    html = (
        "<!DOCTYPE html>"
        '<html lang="en"><head><meta charset="UTF-8">'
        f'<title>EDAV Private Endpoint Monitor v{VERSION} -- Report</title>'
        "<style>"
        "body{font-family:Calibri,Arial,sans-serif;margin:0;color:#333;background:#f4f6f9;}"
        ".header{background:linear-gradient(135deg,#1F4E79 0%,#2980B9 100%);"
        "color:#fff;padding:28px 36px 20px 36px;}"
        ".header h1{margin:0 0 4px 0;font-size:26px;letter-spacing:0.5px;}"
        ".header .sub{font-size:13px;opacity:0.85;margin-top:4px;}"
        ".badges{padding:18px 36px 10px 36px;background:#fff;"
        "border-bottom:1px solid #e0e0e0;display:flex;flex-wrap:wrap;gap:10px;}"
        ".badge{display:inline-flex;align-items:center;gap:6px;padding:8px 16px;"
        "border-radius:6px;font-weight:bold;font-size:13px;box-shadow:0 1px 3px rgba(0,0,0,.12);}"
        ".badge .num{font-size:20px;font-weight:900;}"
        ".green{background:#C6EFCE;color:#276221;}"
        ".red{background:#FFC7CE;color:#9C0006;}"
        ".blue{background:#DDEBF7;color:#1F4E79;}"
        ".grey{background:#D9D9D9;color:#595959;}"
        ".amber{background:#FFEB9C;color:#9C6500;}"
        ".content{padding:24px 36px;}"
        "h2{color:#1F4E79;margin:28px 0 8px 0;font-size:16px;border-bottom:2px solid #DDEBF7;"
        "padding-bottom:6px;}"
        "table{border-collapse:collapse;width:100%;margin-top:4px;font-size:12.5px;"
        "box-shadow:0 1px 4px rgba(0,0,0,.08);border-radius:4px;overflow:hidden;}"
        "th{background:#1F4E79;color:#fff;padding:9px 12px;text-align:left;}"
        "td{padding:7px 12px;border-bottom:1px solid #eee;}"
        "tr:hover td{filter:brightness(0.96);}"
        ".footer{margin-top:32px;padding:16px 36px;background:#f0f4f8;"
        "border-top:1px solid #ddd;font-size:11px;color:#888;}"
        "</style></head><body>"
        '<div class="header">'
        f'<h1>EDAV Private Endpoint Monitor <span style="opacity:0.75;font-weight:400">v{VERSION}</span></h1>'
        f'<div class="sub">Run Date: {run_date}&nbsp;&nbsp;|&nbsp;&nbsp;Mode: {mode}'
        + (f"&nbsp;&nbsp;|&nbsp;&nbsp;Change Ticket: {change_ticket}" if change_ticket else "")
        + (f"&nbsp;&nbsp;|&nbsp;&nbsp;Approved By: {approved_by}" if approved_by else "")
        + "</div></div>"
        '<div class="badges">'
        f'<div class="badge grey"><span class="num">{len(results)}</span> Total Scanned</div>'
        f'<div class="badge amber"><span class="num">{disconnected}</span> Disconnected</div>'
        f'<div class="badge green"><span class="num">{safe}</span> Safe Delete Candidates</div>'
        f'<div class="badge blue"><span class="num">{deleted}</span> Deleted</div>'
        f'<div class="badge green"><span class="num">{verified}</span> Verified Gone</div>'
        + (f'<div class="badge red"><span class="num">{failed}</span> Delete Failed</div>' if failed else "")
        + (f'<div class="badge red"><span class="num">{ver_failed}</span> Verify Failed</div>' if ver_failed else "")
        + '</div>'
        '<div class="content">'
        "<h2>Action Distribution</h2>"
        "<table><thead><tr><th>Recommended Action</th><th>Count</th><th>%</th></tr></thead>"
        f"<tbody>{summary_table_html()}</tbody></table>"
        ...
        f"<tbody>{scan_rows_html()}</tbody></table>"
        ...
        f"</tr></thead><tbody>{del_rows_html()}</tbody></table>"
        ...
        f"</tr></thead><tbody>{ver_rows_html()}</tbody></table>"
        "</div>"
        '<div class="footer">'
        f"Generated by EDAV Private Endpoint Monitor v{VERSION} &nbsp;|&nbsp; "
...
def build_email_html(results: list, del_log: list, verify_log: list, run_date: str) -> str:
    ...
    return (
        ...
        f"<p><strong>Deleted:</strong> {deleted}  |  "
...
def print_executive_dashboard(results: list, del_log: list, verify_log: list,
    ...
    """Print an executive dashboard to console at end of run."""
    total_scanned    = len(results)
    total_disconnected = sum(1 for r in results if r.get("Connection State") == "Disconnected")
    total_approved   = sum(1 for r in del_log)
    total_deleted    = sum(1 for r in del_log if r.get("Delete Result") == "Deleted")
    total_dry_run    = sum(1 for r in del_log if r.get("Delete Result") == "Dry-Run")
    total_failed     = sum(1 for r in del_log if r.get("Delete Result") == "Failed")
    total_skipped    = sum(1 for r in del_log
                          if r.get("Delete Result") in ("Skipped", "Excluded", "Denied"))
    total_verified   = sum(1 for v in verify_log
                          if v.get("Verification Status", "").startswith("Verified"))
    total_ver_failed = sum(1 for v in verify_log
                          if "FAILED" in v.get("Verification Status", ""))
    excluded         = sum(1 for r in results
                          if r.get("Recommended Action") in ("Excluded", "Denied"))
    safe_candidates  = sum(1 for r in results
    ...
    log.info("-" * W)
    log.info("  DISCOVERY")
    log.info("  Total Endpoints Scanned  : %4d", total_scanned)
    log.info("  Total Disconnected       : %4d", total_disconnected)
    log.info("  Safe Delete Candidates   : %4d", safe_candidates)
    log.info("  Total Excluded / Denied  : %4d", excluded)
    log.info("-" * W)
    log.info("  DELETION")
    log.info("  Total Approved for Delete: %4d", total_approved)
    if total_dry_run:
...
def print_run_summary(output_files: list, log_path: str, elapsed: float):
    """Print a clean footer listing every output file with its size."""
    W = 72
    log.info("")
    log.info("=" * W)
...
def main():
    ...
    # Safety gate: --cleanup-approved requires --delete-approved
    ...
    try:
        ...
        if args.terraform_path:
            log.info("  Terraform loaded from: %s", args.terraform_path)
        ...
        # ----------------------------------------------------------------
        # PHASE 3: CLEANUP ENGINE (only in --cleanup-approved mode)
        # ----------------------------------------------------------------
        del_log    = []
        verify_log = []
        ...
        if args.cleanup_approved:
            ...
            if safe_n:
                ...
                log.info("  >>> Step 1: Open the XLSX, mark ApprovedToDelete=Yes on approved rows")
                log.info("  >>> Step 2: Get change ticket approval (--change-ticket CHGxxxxxxx)")
                log.info("  >>> Step 3: --cleanup-approved --delete-approved --dry-run  (preview)")
                log.info("  >>> Step 4: --cleanup-approved --delete-approved            (execute)")
        ...
        log.info("")
        log.info("Run interrupted by user (Ctrl+C). Partial results may have been saved.")
        sys.exit(0)
    except SystemExit:
        raise
    except Exception as exc:
        log.exception("UNEXPECTED ERROR: %s", exc)
        log.error("Run terminated unexpectedly. Check the log file for details: %s", log_path)
        sys.exit(2)
